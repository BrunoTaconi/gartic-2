...
import { MOCKED_PLAYERS } from "./app/components/data";
...
function startNewTurn(roomId: string) {
  ...
  if (!room || room.players.length < 3) {
    // Garante que há jogadores suficientes para começar
    if (room) {
      broadcastToRoom(roomId, {
        type: "ERROR",
        payload: {
          message:
            "São necessários pelo menos 3 jogadores para iniciar um novo turno.",
        },
      });
    }
    return;
  ...
  // MELHORIA: Sorteia os desenhistas aleatoriamente
  const playersCopy = [...room.players];
  const drawer1Index = Math.floor(Math.random() * playersCopy.length);
  const drawer1Conn = playersCopy.splice(drawer1Index, 1)[0];

  const drawer2Index = Math.floor(Math.random() * playersCopy.length);
  const drawer2Conn = playersCopy.splice(drawer2Index, 1)[0];

  // Atualiza os papéis dos jogadores
  room.players.forEach((p) => {
    const player = MOCKED_PLAYERS[p.playerId];
    if (player) {
      if (p.playerId === drawer1Conn.playerId) {
        player.role = "DRAWER_1";
      } else if (p.playerId === drawer2Conn.playerId) {
        player.role = "DRAWER_2";
      } else {
        player.role = "GUESSER";
      }
    }
  ...
  const newGame: GameState = {
    word: newWord,
    drawer1: { id: drawer1Conn.playerId, word: newWord.parts[0] },
    drawer2: { id: drawer2Conn.playerId, word: newWord.parts[1] },
    guesses: [],
    score: 0,
    partsGuessed: [],
  };
      ...
      case "JOIN_ROOM": {
        ...
        if (!roomId || !playerId) {
          ws.send(
            JSON.stringify({
              type: "ERROR",
              payload: { message: "ID da sala e do jogador são obrigatórios." },
            })
          );
          return;
        }
        ...
        if (room.players.some((p) => p.playerId === playerId)) {
          ws.send(
            JSON.stringify({
              type: "ERROR",
              payload: { message: `Jogador ${playerId} já está na sala.` },
            })
          );
          return;
        }
